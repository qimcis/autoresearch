...
def _select_replay_request_window(
    ...
    total_output_tokens = sum(len(request.output_ids) for request in request_history_window)
    target_requests = max(24, min(len(request_history_window), (len(request_history_window) + 2) // 3))
    target_insert_events = max(1, min(total_insert_events, (total_insert_events + 1) // 2))
    target_output_tokens = max(256, min(total_output_tokens, total_output_tokens // 3))
    ...
    for request in reversed(request_history_window):
        ...
        if (
            len(suffix) >= target_requests
            and suffix_insert_events >= target_insert_events
            and suffix_output_tokens >= target_output_tokens
        ):
            break
...
def tune_marconi_eff_weight(
    *,
    ...
    replay_window = _select_replay_request_window(request_history_window)
